...
def transform_date_published_to_python_date(movie):
    try:
        ...
        movie["date_published"] = date.fromisoformat(f"{movie['date_published']}-01-01")
...
def _transform_missing_item_to_unkown(movie, key):
    value = movie.get(key)
    movie[key] = value if isinstance(value, str) and value.strip() else "Unknown"
...
def transform_budget_value_and_currency_or_none(movie):
    ...
    if not match:
        match = re.match(r"(?P<currency>\S+)\s+(?P<value>\d+)", f"USD {budget}")
