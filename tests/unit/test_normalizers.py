...
class TestNormalizers:
    ...
    def test_transform_missing_fields_to_unkown_converts_empty_strings(self, field):
    ...
    def test_transform_missing_fields_to_unkown_keeps_non_empty_strings(self, field):
        """Verifies than given a call to function
        transform_missing_fields_to_unkown, when if received a dictionnary
        entry with an empty string value, then value is set to 'Unknown'."""
        dic = {field: "aBc"}
...
class TestMovieNormalizer:
    ...
    def test_normalize_all_calls_normalize_as_expected(self, mocker, movies):
        ...
        times."""
        mock_normalize = mocker.patch("movies.normalizers.MovieNormalizer.normalize")
    ...
    @pytest.mark.parametrize("normalizers", [[Mock()], [Mock() for _ in range(10)]])
    ...
    def test_all_normalizers_are_registered_to_movie_normalizer(self):
        ...
        normalizers = [
            getattr(norm, elt) for elt in dir(norm) if elt.startswith("transform")
        ]
